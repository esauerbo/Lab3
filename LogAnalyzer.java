...
public class LogAnalyzer
{
      ...
      )
   {
      ...
      //does words[view_session_id] return a list of three strings
      //relate session to product, session id is key in hash table
      //
      View view = new View(words[VIEW_SESSION_ID], words[VIEW_PRODUCT_ID], Double.parseDouble(words[VIEW_PRICE]));
      ...
      )
   {
      ...
      //relate session to product, session id is key in hash table
      //
      Buy buy = new Buy(words[BUY_SESSION_ID], words[BUY_PRODUCT_ID], Double.parseDouble(words[BUY_PRICE]),
              Double.parseDouble(words[BUY_QUANTITY]));
      ...
      )
   {
      ...
      for (Map.Entry<String, List<String>> customer : sessionsFromCustomer.entrySet()) {
         List<String> sessions = (List<String>)customer.getValue();
         for (String sessionId : sessions) {
            if (buysFromSession.containsKey(sessionId)){
               ...
               for (View view : views){
                  numviews++;
                  sessiontot = sessiontot + view.getPrice();
               }
               double avg = sessiontot/numviews;
               List<Buy> buys = buysFromSession.get(sessionId);
               for (Buy buy : buys){
                  double diff = buy.getPrice()-avg;
                  System.out.println("    "+buy.getProductId()+ " " + diff);
   ...
   private static void printCustomerItemViewsForPurchase(
      ...
      for (Map.Entry<String, List<String>> customer : sessionsFromCustomer.entrySet()) {
         List<String> sessions = customer.getValue();
         List<Buy> purchases = new LinkedList<>();
         for (String sessionId : sessions) {
            if (buysFromSession.containsKey(sessionId)){
               List<Buy> buys = buysFromSession.get(sessionId);
               for (Buy buy : buys){
         ...
         for (Buy buy: purchases){
            ...
            for (String sessionId : sessions){
               ...
               List<View> viewslist = viewsFromSession.get(sessionId);
               if (viewslist != null){
            ...
            System.out.println("    "+buy.getProductId()+" "+numviews);
         }
   ...
   private static void printCustomerItemViewsWithoutPurchase(
           final Map<String, List<String>> sessionsFromCustomer,
           final Map<String, List<View>> viewsFromSession,
           final Map<String, List<Buy>> buysFromSession
   ) {
      ...
      for (Map.Entry<String, List<String>> customer : sessionsFromCustomer.entrySet()) {
         ...
         for (String sessionId : sessions) {
            ...
            List<View> views = viewsFromSession.get(sessionId);
            if (views != null) {
               totalviews = totalviews + views.size();
            }
         }
         }
      }
      double avg = totalviews/numsessions;
      System.out.println("Average Views without Purchase: " + avg);
   }
   ...
   {
      printCustomerItemViewsWithoutPurchase(sessionsFromCustomer, viewsFromSession, buysFromSession);
      ...
      )
   {
      ...
      {
         processLine(input.nextLine(), sessionsFromCustomer, viewsFromSession,
            buysFromSession);
